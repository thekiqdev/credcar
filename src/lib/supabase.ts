...
export const tempPDVGeneratorService = {
  async generatePDVForAllRepresentatives() {
    try {
      console.log("🚀 Iniciando geração de PDV para todos os representantes...");
      
      // Buscar todos os representantes que não têm PDV
      const { data: representatives, error: fetchError } = await supabase
        .from("profiles")
        .select("id, full_name, point_of_sale")
        .eq("role", "Representante")
        .is("point_of_sale", null);

      if (fetchError) {
        console.error("❌ Erro ao buscar representantes:", fetchError);
        throw fetchError;
      }

      console.log(`📊 Encontrados ${representatives?.length || 0} representantes sem PDV`);

      if (!representatives || representatives.length === 0) {
        console.log("✅ Todos os representantes já possuem PDV!");
        return { success: true, message: "Todos os representantes já possuem PDV", count: 0 };
      }

      // Buscar todos os PDVs existentes para evitar duplicatas
      const { data: existingPDVs, error: existingError } = await supabase
        .from("profiles")
        .select("point_of_sale")
        .eq("role", "Representante")
        .not("point_of_sale", "is", null);

      if (existingError) {
        console.error("❌ Erro ao buscar PDVs existentes:", existingError);
        throw existingError;
      }

      const existingCodes = new Set((existingPDVs || []).map(p => p.point_of_sale));
      console.log(`📋 PDVs existentes: ${existingCodes.size}`);
      ...
      for (const rep of representatives) {
        try {
          ...
          do {
            pdvCode = this.generatePDVCode();
            attempts++;
          } while (existingCodes.has(pdvCode) && attempts < maxAttempts);

          if (attempts >= maxAttempts) {
            console.error(`❌ Não foi possível gerar PDV único para ${rep.full_name} após ${maxAttempts} tentativas`);
            errorCount++;
            continue;
          }

          // Adicionar ao conjunto de códigos existentes
          existingCodes.add(pdvCode);
          ...
          if (updateError) {
            console.error(`❌ Erro ao atualizar PDV para ${rep.full_name}:`, updateError);
            errorCount++;
          } else {
            console.log(`✅ PDV gerado para ${rep.full_name}: ${pdvCode}`);
            successCount++;
          }

        } catch (error) {
          console.error(`❌ Erro ao processar ${rep.full_name}:`, error);
          errorCount++;
        }
      }
      ...
      console.error("❌ Erro geral na geração de PDV:", error);
      throw error;
    }
  },
  ...
  generatePDVCode() {
    ...
    let numbersStr = '';
    for (let i = 0; i < 6; i++) {
      numbersStr += numbers[Math.floor(Math.random() * numbers.length)];
    }
    
    return `PV-${randomLetter}${numbersStr}`;
  }
};
