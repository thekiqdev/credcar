/**
 * Componente de Upload de Documentos de Sócios
 * Interface específica para upload de documentos de sócios individuais
 */
...
const PartnerDocumentUpload: React.FC<PartnerDocumentUploadProps> = ({
  partnerId,
  partnerName,
  partnerCpf,
  representativeId,
  ...
  const [documents, setDocuments] = useState<DocumentFile[]>([
    ...
    { id: '1', type: 'cartilha de credenciamento pf', file: null, status: 'pending', progress: 0 },
    { id: '2', type: 'comprovante de endereço em nome do sócio', file: null, status: 'pending', progress: 0 },
    { id: '3', type: 'certidão de antecedentes criminais', file: null, status: 'pending', progress: 0 },
    { id: '4', type: 'certidão negativa cível de 1º grau', file: null, status: 'pending', progress: 0 },
    { id: '5', type: 'certidão negativa criminal de 1º grau', file: null, status: 'pending', progress: 0 },
    { id: '6', type: 'foto de identidade ou cnh (frente)', file: null, status: 'pending', progress: 0 },
    { id: '7', type: 'foto de identidade ou cnh (verso)', file: null, status: 'pending', progress: 0 }
  ]);
  ...
  const loadDocumentStatus = async () => {
    try {
      console.log('📋 PartnerDocumentUpload: loadDocumentStatus chamado');
      setIsLoading(true);
      ...
      console.log('📋 Documentos do sócio carregados do banco:', data);
      ...
      console.error('Error loading partner document status:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const saveToDatabase = async (fileData: any, partnerId: string) => {
    try {
      console.log('💾 === INÍCIO saveToDatabase Partner ===');
    ...
    try {
      ...
      for (const doc of filesToUpload) {
        ...
        try {
          ...
          await saveToDatabase(saveData, partnerId);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-4xl max-h-[85vh] overflow-hidden flex flex-col">
        <CardHeader className="bg-gray-50 border-b">
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Envie os documentos obrigatórios para {partnerName} (CPF: {partnerCpf})
              </CardDescription>
            </div>
            {onClose && (
        ...
        <CardContent className="p-6 overflow-y-auto flex-1">
          ...
          {/* Documents List - Apenas Documentos do Sócio */}
