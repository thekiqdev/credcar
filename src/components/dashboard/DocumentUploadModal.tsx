...
const DocumentUploadModal: React.FC<DocumentUploadModalProps> = ({
      ...
      if (data && data.length > 0) {
        setDocuments(prev => prev.map(doc => {
          ...
          if (doc.type === 'cartilha de credenciamento preenchida') {
            console.log('🔍 === DEBUG CARTILHA LOAD ===');
            ...
            if (dbDoc) {
              console.log('📄 dbDoc COMPLETO:', JSON.stringify(dbDoc, null, 2));
              console.log('📄 dbDoc.id:', dbDoc.id);
              ...
              console.log('📄 dbDoc.file_url.trim() !== "":', dbDoc.file_url?.trim() !== '');
              console.log('📄 dbDoc.uploaded_at:', dbDoc.uploaded_at);
            } else {
              console.log('❌ dbDoc NÃO encontrado!');
              console.log('📋 Todos os tipos no banco:');
              data.forEach(d => console.log(`  - "${d.document_type}"`));
            }
            console.log('🔍 === FIM DEBUG CARTILHA LOAD ===');
          }
  ...
  const saveToDatabase = async (fileData: any, representativeId: string) => {
    try {
      ...
      if (fileData.documentType === 'cartilha de credenciamento preenchida') {
        console.log('🔍 === DEBUG CARTILHA SAVE ===');
        console.log('📄 fileData COMPLETO:', JSON.stringify(fileData, null, 2));
        console.log('📄 fileData.documentType:', fileData.documentType);
        console.log('📄 fileData.filePath:', fileData.filePath);
        console.log('📄 fileData.directory:', fileData.directory);
        console.log('🔍 === FIM DEBUG CARTILHA SAVE ===');
      }
      ...
      if (existingDoc) {
        ...
        // Log específico para cartilha
        if (fileData.documentType === 'cartilha de credenciamento preenchida') {
          console.log('🔍 === DEBUG CARTILHA UPDATE RESULT ===');
          console.log('📄 data:', JSON.stringify(data, null, 2));
          console.log('📄 existingDoc.id:', existingDoc.id);
          console.log('📄 filePath atualizado:', filePath);
          console.log('🔍 === FIM DEBUG CARTILHA UPDATE RESULT ===');
        }
      ...
      // Log específico para cartilha
      if (fileData.documentType === 'cartilha de credenciamento preenchida') {
        console.log('🔍 === DEBUG CARTILHA INSERT RESULT ===');
        console.log('📄 data:', JSON.stringify(data, null, 2));
        console.log('📄 insertData:', JSON.stringify(insertData, null, 2));
        console.log('🔍 === FIM DEBUG CARTILHA INSERT RESULT ===');
      }
    ...
    try {
      ...
      for (const doc of filesToUpload) {
        ...
        try {
          ...
          if (doc.type === 'cartilha de credenciamento preenchida') {
            console.log('🔍 === DEBUG CARTILHA UPLOAD RESULT ===');
            console.log('📄 result.success:', result.success);
            console.log('📄 result.data:', JSON.stringify(result.data, null, 2));
            console.log('📄 result.error:', result.error);
            console.log('🔍 === FIM DEBUG CARTILHA UPLOAD RESULT ===');
          }
          ...
          // Log específico para cartilha antes de salvar
          if (doc.type === 'cartilha de credenciamento preenchida') {
            console.log('🔍 === DEBUG CARTILHA ANTES DE SAVE ===');
            console.log('📄 saveData:', JSON.stringify(saveData, null, 2));
            console.log('📄 saveData.documentType:', saveData.documentType);
            console.log('📄 saveData.filePath:', saveData.filePath);
            console.log('📄 saveData.directory:', saveData.directory);
            console.log('📄 representativeId:', representativeId);
            console.log('🔍 === CHAMANDO saveToDatabase PARA CARTILHA ===');
          }
          ...
          // Log específico para cartilha após salvar
          if (doc.type === 'cartilha de credenciamento preenchida') {
            console.log('🔍 === DEBUG CARTILHA APÓS SAVE ===');
            console.log('✅ saveToDatabase completado com sucesso');
            console.log('🔍 === FIM DEBUG CARTILHA APÓS SAVE ===');
          }
          ...
          // Log específico para cartilha
          if (doc.type === 'cartilha de credenciamento preenchida') {
            console.log('🔍 === DEBUG CARTILHA ERROR ===');
            console.log('❌ Erro capturado:', docError);
            console.log('❌ Erro message:', docError instanceof Error ? docError.message : 'Erro desconhecido');
            console.log('❌ Erro stack:', docError instanceof Error ? docError.stack : undefined);
            console.log('🔍 === FIM DEBUG CARTILHA ERROR ===');
          }
