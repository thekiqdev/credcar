...
// Endpoint para upload de contrato de representante
app.post('/api/upload-representative-contract', upload.single('contract'), async (req, res) => {
  try {
    ...
    if (!req.file) {
      return res.status(400).json({
        success: false,
        message: 'Arquivo de contrato é obrigatório'
      });
    }

    console.log(`📄 Upload de contrato para representante ${representativeId}:`, {
      filename: req.file.filename,
      originalname: req.file.originalname,
      size: req.file.size
    });

    // Criar pasta específica para contratos de representantes
    const contractDir = path.join(__dirname, 'documentos', representativeId, 'contrato-representante');
    
    // Garantir que a pasta existe
    if (!fs.existsSync(contractDir)) {
      fs.mkdirSync(contractDir, { recursive: true });
      console.log(`📁 Pasta criada: ${contractDir}`);
    }

    // Mover arquivo para a pasta específica
    const finalPath = path.join(contractDir, req.file.filename);
    fs.renameSync(req.file.path, finalPath);
    
    console.log(`✅ Arquivo movido para: ${finalPath}`);

    // Criar URL de download
    const downloadUrl = `/api/download-representative-contract/${representativeId}/${req.file.filename}`;
    
    // Atualizar campo contract_profile na tabela profiles
    const { error: updateError } = await supabase
      .from('profiles')
      .update({ contract_profile: downloadUrl })
      .eq('id', representativeId);

    if (updateError) {
      console.error('❌ Erro ao atualizar contract_profile:', updateError);
      return res.status(500).json({
        success: false,
        message: 'Erro ao salvar link do contrato no banco de dados',
        error: updateError.message
      });
    }

    console.log(`✅ Contract profile atualizado para representante ${representativeId}`);

    res.json({
      success: true,
      message: 'Contrato do representante enviado com sucesso',
      data: {
        filename: req.file.filename,
        originalname: req.file.originalname,
        size: req.file.size,
    ...
    console.error('❌ Erro no upload de contrato de representante:', error);
    res.status(500).json({
      success: false,
...
// Endpoint para download de contrato de representante
app.get('/api/download-representative-contract/:representativeId/:filename', async (req, res) => {
  try {
    const { representativeId, filename } = req.params;
    
    const filePath = path.join(__dirname, 'documentos', representativeId, 'contrato-representante', filename);
    ...
    console.log(`📥 Download de contrato: ${filePath}`);
